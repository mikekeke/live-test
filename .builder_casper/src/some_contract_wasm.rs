    ...
    match constructor_name.as_str() {
        stringify!(init) => {
            let odra_address = odra::types::Address::try_from(contract_package_hash)
                ...
                .unwrap_or_revert();
            let mut contract_ref = something::some_contract::SomeContractRef::at(&odra_address);
...
fn init() {
    ...
    let mut _contract = something::some_contract::SomeContract::instance("contract");
    let name =
...
fn add_something() {
    ...
    let mut _contract = something::some_contract::SomeContract::instance("contract");
    let some_text =
        odra::casper::casper_contract::contract_api::runtime::get_named_arg(stringify!(some_text));
    _contract.add_something(some_text);
}
#[no_mangle]
fn get_something() {
    odra::casper::utils::assert_no_attached_value();
    ...
    let result = _contract.get_something(id);
